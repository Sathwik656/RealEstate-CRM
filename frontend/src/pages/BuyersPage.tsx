...
export default function BuyersPage() {
  ...
  if (isCreating) return <CreateBuyer onSuccess={() => { setIsCreating(false); refetch(); }} onCancel={() => setIsCreating(false)} />;

  return (
    <div className="page-wrapper">
      <div className="page-header">
        <div><h1 className="page-title">Buyers</h1><p className="page-subtitle">Manage prospective property buyers</p></div>
        <button className="btn-accent" onClick={() => setIsCreating(true)}><Plus size={16} /> Add Buyer</button>
      </div>
      <div className="card">
        <div className="overflow-x-auto">
          <table className="data-table">
            <thead><tr><th>Buyer ID</th><th>Name</th><th>Contact</th><th>Location</th><th>Budget Max</th><th>Follow-Up</th><th className="text-right">Actions</th></tr></thead>
            <tbody>
              {isLoading ? <tr><td colSpan={7} className="py-12 text-center text-muted">Loading...</td></tr>
               ...
               : data.data.map((b: any) => (
                <tr key={b._id}>
                  <td><span className="font-mono text-xs text-muted">{b.buyerId}</span></td>
